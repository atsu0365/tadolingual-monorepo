...
class WebMedia {
  ...
  calculateSeoScore() {
    let score = 0;
    if (this.metaTitle && this.metaTitle.length <= 60) score += 20;
    if (this.metaDescription && this.metaDescription.length <= 160) score += 20;
    if (this.slug) score += 20;
    if (this.canonicalUrl) score += 20;
    if (this.content.includes(this.focusKeyword)) score += 20;
    return score;
  }
